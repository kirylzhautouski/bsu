...
public:
	static void Main()
	{
		...
		if (process.IsMaster())
		{
			...
			fout.open("output/task2.2_output.txt");
		...
		{
			ReceiveData(slice_ab, slice_size);
		}
...
private:
	...
	static void ReceiveData(int** destination, int slice_size)
	{
		for (int i = 0; i < slice_size; i++)
		{
			MPI_Status status;
			...
			MPI_Get_count(       // 2. Получаем длину данных.
				...
				&data_length);    // Результат.               

			destination[i] = new int[data_length];
			MPI_Recv(destination[i], data_length, MPI_INT, MPI::MasterRank, i, 
